...
import pygame
import subprocess  # Import pour appeler une autre partie (partie2.py)

# Initialisation de Pygame
pygame.init()

# Charger les images
background_image = pygame.image.load("./scripts/footages/pont.png")  # Image pour la simulation principale
image_lm = pygame.image.load("./scripts/footages/LM.png")  # Image pour afficher en haut
image_st = pygame.image.load("./scripts/footages/ST.png")  # Nouvelle image pour l'étape intermédiaire
ecran_image = pygame.image.load("./scripts/footages/ecran.png")  # Image pour l'écran explicatif
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if current_state == STATE_START:
                    ...
                    elif poursuivre_button_rect.collidepoint(event.pos):  # Bouton "Poursuivre"
                        all_correct = all(
                            ((cartesian_to_pixel(*point["target"])[0] - point["pos"][0])**2 + 
                             (cartesian_to_pixel(*point["target"])[1] - point["pos"][1])**2)**0.5 < 10
                            for point in points.values()
                        )
                        if all_correct:
                            pygame.quit()  # Quitte la fenêtre actuelle
                            subprocess.run(["python", "partie2.py"])  # Lancer partie2.py
                            return
        ...
        # Simulation principale
            ...
            # Vérifier si tous les points sont correctement placés
            all_correct = True
            for name, point in points.items():
            ...
            # Dessiner les points (blancs si pas tous corrects, verts sinon)
            for name, point in points.items():
